...
var _promises = _interopRequireDefault(require("./libs/promises"));
...
function _default(source) {
  ...
  var tags = (0, _getTags.default)(options, source);
  Promise.all((0, _promises.default)(this, tags)).then(function (res) {
    res.forEach(function (item) {
      var url = item.source.match(/(?<=^module\.exports[\s]=[\s])[\s\S]*/g)[0];

      if (/^__webpack_public_path__/.test(url)) {
        url = _path.default.resolve(pubPath, url.split(' + ')[1].slice(1, -2));
    ...
    if (options.extract) {
      ...
      callback(null, "export default ".concat(JSON.stringify(source)));
    }
  }, function (err) {
    return callback(err);
  });
